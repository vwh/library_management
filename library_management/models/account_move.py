...
class AccountMoveLine(models.Model):
    ...
    def reconcile(self):
        ...
        _logger.info(
            f"=== Before reconciliation: Found {len(moves_to_check)} invoices to check ==="
        )
    ...
    def _activate_related_memberships(self):
        for move in self:
            if move.move_type == "out_invoice" and move.payment_state == "paid":
                ...
                _logger.info(
                    f"Found {len(related_memberships)} memberships to activate for partner {move.partner_id.name}"
                )

                for membership in related_memberships:
                    _logger.info(
                        f"Activating membership {membership.id} for member {membership.partner_id.name}"
                    )
                    membership.write(
                        {
                            "state": "active",
