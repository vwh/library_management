...
class ResPartner(models.Model):
    ...
    membership_ids = fields.One2many(
        "library.membership", "partner_id", string="Memberships"
    )

    # Computed field to show if partner has any active memberships
    is_member = fields.Boolean(
        string="Active Member", compute="_compute_is_member", store=True
    )
    ...
    def _compute_is_member(self):
        ...
        for partner in self:
            ...
            partner.is_member = any(
                membership.state == "active" for membership in partner.membership_ids
            )
